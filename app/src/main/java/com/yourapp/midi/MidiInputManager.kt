...
class MidiInputManager @Inject constructor(@ApplicationContext private val context: Context) {
    ...
    fun connectFirstAvailableDevice(): Boolean {
        ...
        val info = devices.filter { it.outputPortCount > 0 }
            .sortedByDescending {
                val name = it.properties.getString(MidiDeviceInfo.PROPERTY_NAME) ?: ""
                if (name.contains("E343", true) || name.contains("Yamaha", true)) 1 else 0
            }.firstOrNull()
        if (info == null) {
    ...
    fun connect(info: MidiDeviceInfo) {
        val mgr = midiManager ?: run { DebugLog.add("❌ MidiManager null"); return }
        close()
        ...
        mgr.openDevice(info, { device ->
            if (device == null) { DebugLog.add("❌ openDevice returned null"); return@openDevice }
            openedDevice = device
            var connectedInputs = 0
            for (portIndex in 0 until info.outputPortCount) {
                try {
                    val recvPort = device.openOutputPort(portIndex)
                    if (recvPort == null) { DebugLog.add("⚠ MIDI IN port $portIndex = null"); continue }
                    recvPort.connect(MidiNoteReceiver(portIndex))
            ...
            if (info.inputPortCount > 0) {
                try {
                    ...
                    if (sendPort != null) { inputPort = sendPort; DebugLog.add("✅ MIDI OUT port 0 ready") }
                    else DebugLog.add("⚠ MIDI OUT port 0 = null")
                } catch (e: Exception) { DebugLog.add("❌ MIDI OUT open: ${e.javaClass.simpleName}") }
            } else DebugLog.add("ℹ Device has no INPUT port")
    ...
    private inner class MidiNoteReceiver(private val portIndex: Int) : MidiReceiver() {
        ...
        override fun onSend(data: ByteArray, offset: Int, count: Int, timestamp: Long) {
            if (!rawLogged) {
                val bytes = (offset until offset + count).joinToString(" ") { "%02X".format(data[it].toInt() and 0xFF) }
        ...
        private fun parseMessages(data: ByteArray, offset: Int, count: Int) {
            ...
            while (i < end) {
                ...
                if (b and 0x80 != 0) {
                    ...
                    if (b >= 0xF0) {
                        when (b) {
                            0xF0 -> while (i < end) { val sb = data[i++].toInt() and 0xFF; if (sb == 0xF7) break }
                ...
                if (status < 0 || status >= 0xF0) { i++; continue }
                val type = status and 0xF0
                if (type == 0xC0 || type == 0xD0) { if (i < end) i++; continue }
                if (pendingData1 < 0) { if (i >= end) break; pendingData1 = data[i++].toInt() and 0x7F }
                if (i >= end) break
                val d1 = pendingData1
                val d2 = data[i++].toInt() and 0x7F
                pendingData1 = -1
                when (type) {
                    0x90 -> if (d2 > 0) { onNoteOn?.invoke(d1, d2); DebugLog.add("🎹 IN NoteOn port=$portIndex n=$d1 vel=$d2") }
                    else { onNoteOff?.invoke(d1); DebugLog.add("🎹 IN NoteOff port=$portIndex n=$d1") }
                    0x80 -> { onNoteOff?.invoke(d1); DebugLog.add("🎹 IN NoteOff port=$portIndex n=$d1") }
                }
            }
        }
    }
    ...
    /** Legacy API: bank is a 7-bit MSB only value. */
    fun sendProgramChange(channel: Int, program: Int, bank: Int = 0) =
        sendProgramChangeBank(channel, program, bankMsb = bank.coerceIn(0, 127), bankLsb = 0)

    /** Yamaha/XG-safe 14-bit bank select: CC0=MSB, CC32=LSB, then PC. */
    fun sendProgramChangeBank(channel: Int, program: Int, bankMsb: Int, bankLsb: Int) {
        val port = inputPort ?: return
        val ch = channel.coerceIn(0, 15)
        val msb = bankMsb.coerceIn(0, 127)
        val lsb = bankLsb.coerceIn(0, 127)
        try {
            port.send(byteArrayOf((0xB0 or ch).toByte(), 0, msb.toByte()), 0, 3)
            port.send(byteArrayOf((0xB0 or ch).toByte(), 32, lsb.toByte()), 0, 3)
            port.send(byteArrayOf((0xC0 or ch).toByte(), program.coerceIn(0, 127).toByte()), 0, 2)
        } catch (e: Exception) { Timber.w(e, "sendProgramChangeBank failed") }
    }

    fun allNotesOff() {
        val port = inputPort ?: return
        for (ch in 0 until 16) try { port.send(byteArrayOf((0xB0 or ch).toByte(), 123.toByte(), 0), 0, 3) } catch (_: Exception) {}
    }

    fun close() {
        outputPorts.forEach { try { it.close() } catch (e: Exception) { Timber.w(e, "close outputPort") } }
        outputPorts.clear()
