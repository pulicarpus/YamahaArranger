...
class StyleSequencer(
    ...
    private fun applyVoicesFromCasm(section: StyleSectionModel) {
        ...
        section.parts.forEachIndexed { idx, part ->
            ...
            // RIGHT 1/2/3 permanently own synth channels 0/1/2.
            // Style CASM must never reprogram these channels; otherwise
            // changing/starting a style can silently replace the keyboard
            // voice or leave the layer on a preset that does not exist.
            if (ch in 0..2) {
                DebugLog.add("  · part$partNum ch$ch: SKIP (reserved for RIGHT 1/2/3)")
