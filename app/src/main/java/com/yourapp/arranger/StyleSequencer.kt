...
class StyleSequencer(private val audioEngine: AudioEngineManager, private val midiInputManager: MidiInputManager, private val scope: CoroutineScope) {
    ...
    private fun applyVoicesFromCasm(section:StyleSectionModel){section.parts.forEach{part->val c=part.casm?:return@forEach;val destination=c.destinationChannel;if(destination in lockedChannels)return@forEach;val prog=guessProgramFromVoiceName(c.voiceName);if(prog<0){DebugLog.add("⚠ src${c.sourceChannel}→dst$destination: unsupported '${c.voiceName}'");return@forEach};val drum=destination==9||isDrumVoice(c.voiceName);val bank=if(drum)128 else 0;audioEngine.setChannelProgram(destination,prog,bank);midiInputManager.sendProgramChange(destination,prog,bank);DebugLog.add("🎼 src${c.sourceChannel}→dst$destination: ${c.voiceName} → GM $prog bank=$bank NTR=${c.ntr} NTT=${c.ntt} HK=${c.highKey} LIM=${c.noteLimitLow}..${c.noteLimitHigh} RTR=${c.rtr}")}}
    ...
    private suspend fun playOnce(section:StyleSectionModel,ppq:Int){loopCount++;if(section.lengthTicks<=0){delay(500);return};data class Scheduled(val tick:Int,val event:StyleNoteEvent,val policy:CasmPolicyModel?);val merged=section.parts.flatMap{part->part.events.filter(::isNoteEvent).map{e->Scheduled(e.tick,e,part.casm)}}.sortedWith(compareBy<Scheduled>{it.tick}.thenBy{it.event.isNoteOn.not()});if(merged.isEmpty()){delay(500);return};var lastTick=0;for(s in merged){val delta=s.tick-lastTick;if(delta>0)delay(ticksToMillis(delta,ppq,tempoBpm));lastTick=s.tick;val policy=s.policy;val chord=currentChord;if(policy!=null&&chord!=null&&!policyMatchesChord(policy,chord))continue;val sourceChannel=s.event.channel;val destinationChannel=policy?.destinationChannel?:sourceChannel;if(destinationChannel in lockedChannels)continue;val key="${sourceChannel}:${destinationChannel}:${s.event.note}";if(!s.event.isNoteOn){val releaseNote=activeTransposedNotes.remove(key);if(releaseNote!=null){audioEngine.noteOffChannel(destinationChannel,releaseNote);midiInputManager.sendNoteOff(destinationChannel,releaseNote)};continue};val isDrumPart=destinationChannel==9||(policy!=null&&isDrumVoice(policy.voiceName));val transformed=if(policy!=null&&!isDrumPart){chord?.let{CasmNoteTransformer.transform(s.event.note,it,policy)}?:s.event.note.coerceIn(0,127)}else{s.event.note.coerceIn(0,127)};val note=transformed?:continue;val velocity=s.event.velocity.coerceIn(1,127);activeTransposedNotes[key]=note;audioEngine.noteOnChannel(destinationChannel,note,velocity/127f);midiInputManager.sendNoteOn(destinationChannel,note,velocity)};val rem=section.lengthTicks-lastTick;if(rem>0)delay(ticksToMillis(rem,ppq,tempoBpm))}
