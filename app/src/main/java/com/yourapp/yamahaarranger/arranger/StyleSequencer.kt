...
class StyleSequencer(
    ...
    fun play(
        ...
        DebugLog.add(
            "▶ PLAY ${section.name}: parts=${section.parts.size}, " +
                "events=${section.parts.sumOf { it.events.size }}, loopLimit=$loopLimit"
        )
    ...
    private fun applyVoicesFromCasm(section: StyleSectionModel) {
        section.parts.forEach { part ->
            ...
            DebugLog.add(
                "🎼 src${c.sourceChannel}→dst$destination: ${c.voiceName} → GM $prog bank=$bank " +
                    "NTR=${c.ntr} NTT=${c.ntt} BASS=${c.bassOn} SCT=${c.sourceChordType} " +
                    "HK=${c.highKey} LIM=${c.noteLimitLow}..${c.noteLimitHigh} RTR=${c.rtr}"
            )
        }
    }
    ...
    private fun guessProgramFromVoiceName(name: String): Int {
        ...
        return when {
            ...
            n.contains("flute") -> 73
            n.contains("crash") || n.contains("cymbal") || n.contains("perc") ||
    ...
    /**
     * CASM can place different source phrases on different channels for the
     * same section and select them by source chord type. LoveSong3, for
     * example, has BassMaj (type 2) and BassMin (type 10) in Intro/Ending
     * groups. Both channels exist in the MIDI data, but only the matching
     * phrase should sound for the current chord.
     */
    private fun policyMatchesChord(policy: CasmPolicyModel, chord: DetectedChord?): Boolean {
        ...
        return when (policy.sourceChordType) {
            0 -> true
            2 -> chord.quality !in setOf(
                ChordQuality.MINOR, ChordQuality.MIN6, ChordQuality.MIN7,
                ChordQuality.DIM, ChordQuality.DIM7
            )
            10 -> chord.quality in setOf(
                ChordQuality.MINOR, ChordQuality.MIN6, ChordQuality.MIN7,
                ChordQuality.DIM, ChordQuality.DIM7
            )
            else -> true
    ...
    private suspend fun playOnce(section: StyleSectionModel, ppq: Int) {
        ...
        for (s in merged) {
            ...
            if (!s.event.isNoteOn) {
                // Always release a note that was actually started. Do not
                // re-evaluate CASM after a chord change: the note-off must use
                // the transformed pitch saved at note-on time.
            ...
            // Source chord type is a selection rule, not a note transform.
            // It must be checked before starting the event; note-offs above
            // remain unconditional so chord changes cannot leave hanging notes.
            if (policy != null && !policyMatchesChord(policy, currentChord)) continue

            val isDrumPart = destinationChannel == 9 ||
    ...
    private fun ticksToMillis(ticks: Int, ppq: Int, bpm: Int): Long =
        if (ppq <= 0 || bpm <= 0) 0
        else ((ticks * (60000.0 / bpm)) / ppq).toLong().coerceAtLeast(0)
}
