    ...
    private fun applyVoicesFromCasm(section: StyleSectionModel) {
        ...
        section.parts.forEach { part ->
            ...
            if (destination in 0..3) {
                com.yourapp.yamahaarranger.ui.DebugLog.add(
                    "  · style dst" + destination + ": SKIP (reserved for keyboard voices)"
                )
                return@forEach
            }
        ...
        section.parts.forEach { part ->
            ...
            if (override?.muted == true) {
                applied += destination
                com.yourapp.yamahaarranger.ui.DebugLog.add("🔇 dst" + destination + " muted by style editor")
                return@forEach
            }
            ...
            if (prog !in 0..127) {
                com.yourapp.yamahaarranger.ui.DebugLog.add(
                    "⚠ src" + c.sourceChannel + "→dst" + destination +
                        ": no usable program for '" + c.voiceName + "'"
                )
                return@forEach
            }

            // Yamaha Bank Select is MSB:LSB. FluidSynth program_select takes
            // the corresponding 14-bit bank number directly.
