...
class StyleSequencer(private val audioEngine: AudioEngineManager, private val scope: CoroutineScope) {
    ...
    fun setVoiceMap(vm: Map<Int, String>) { lastAppliedSection = ""; DebugLog.add("🎼 Legacy VoiceMap received: ${vm.size} entries; section CASM takes precedence") }
    ...
    private fun applyVoicesFromCasm(section: StyleSectionModel) {
        section.parts.forEach { part ->
            ...
            if (prog < 0) {
                DebugLog.add("⚠ src${c.sourceChannel}→dst$destination: unsupported CASM voice '${c.voiceName}'")
                return@forEach
            }
    ...
    private suspend fun playOnce(section: StyleSectionModel, ppq: Int) {
        ...
        for (s in merged) {
            ...
            val note = applyCasmlimits(transformed, policy)
