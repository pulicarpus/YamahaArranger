...
void StyleParser::parseCasm(const uint8_t* data, size_t size) {
    ...
    // Cntt is global to the CASM channel definitions. In real Yamaha files
    // the bass-on flag is bit 7 of the NTT byte here (e.g. 0x81), not Ctab byte 21.
            ...
            while (q + 8 <= payloadEnd) {
                ...
                if (std::memcmp(subTag, "Sdec", 4) == 0) {
                    ...
                    while (begin <= text.size()) {
                        ...
                        std::string item = trimAscii(text.substr(
                ...
                if (std::memcmp(subTag, "Cntt", 4) == 0 && subLen >= 2) {
                    const uint8_t* d = data + subStart;
                    const uint8_t destination = d[0];
                    const uint8_t nttAndFlags = d[1];
                    bassOnByDestination[destination] = (nttAndFlags & 0x80) != 0;
                }
    ...
    // Apply Cntt bass-on after all CSEG/Ctab records have been collected.
    for (auto& [section, secData] : sections_) {
        for (auto& part : secData.parts) {
            for (auto& policy : part.casmPolicies) {
                auto it = bassOnByDestination.find(policy.destinationChannel);
                if (it != bassOnByDestination.end()) policy.bassOn = it->second;
            }
            if (!part.casmPolicies.empty()) part.casm = part.casmPolicies.front();
        }
    }
}
    ...
    for (const auto& track : smf_.tracks()) {
        ...
        for (size_t i = 0; i < boundaries.size(); ++i) {
            ...
            for (const auto& ev : track.events) {
                ...
                if (ev.status == 0xFF && (ev.metaType == 0x06 || ev.metaType == 0x01 || ev.metaType == 0x2F)) continue;
                if (ev.status < 0x80) continue;
                byChannel[ev.channel].push_back(ev);
            }
            ...
            for (auto& [channel, events] : byChannel) {
                ...
                for (auto& ev : events) ev.tick -= startTick;
                part.events = std::move(events);

                // Some styles repeat the voice setup at the beginning of a section.
                // Preserve it when present; the global SInt setup is handled by the
                // next runtime voice-map pass.
...
StyleSection styleSectionFromString(const std::string& s) {
    static const std::map<std::string, StyleSection> m = {
    ...
    auto it = m.find(s);
    return it != m.end() ? it->second : StyleSection::Unknown;
}
